...
def smart_hedging(symbol):
    equity_price = kite.ltp(f"NSE:{symbol}")[f"NSE:{symbol}"]["last_price"]
    atm_strike = get_atm_strike(symbol)
    expiry = "2024-03-28"

    # Buy Equity Position
    place_order(symbol, "BUY", 50)

    # Hedge with ATM Put Option
    hedge_put = f"{symbol}{expiry}{atm_strike}PE"
    place_options_order(hedge_put, "BUY", 50)

    print(f"Smart Hedging: Bought {symbol} and hedged with {hedge_put}")

# Run Smart Hedging
smart_hedging("RELIANCE")
